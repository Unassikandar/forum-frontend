...
contract('ArgetherToken', function(accounts) {
    ...
    it('initializes the contract with the correct values', function() {
        return ArgetherToken.deployed().then(function(instance) {
            ...
            assert.equal(name, 'Argether Token', 'has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'ARG', 'has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard) {
            assert.equal(standard, 'Argether Token v1.0', 'has the correct standard');
        });
    });
    ...
    it('transfers token ownership', function() {
        return ArgetherToken.deployed().then(function(instance) {
            tokenInstance = instance;
            return tokenInstance.transfer.call(accounts[1], 20000000);
    ...
    it('approves tokens for delegated transfer', function() {
        return ArgetherToken.deployed().then(function(instance) {
            ...
            assert.equal(receipt.logs[0].args._owner, accounts[0], 'logs the owner\'s account');
            assert.equal(receipt.logs[0].args._spender, accounts[1], 'logs the spender\'s account');
    ...
    it('handles delegated transfers', function() {
        return ArgetherToken.deployed().then(function(instance) {
            ...
            // try transffering more tokens than the sender's balance
            ...
            assert(error.message.toString().indexOf('revert') >= 0, 'cannot transfer amount larger than the approved amount');
